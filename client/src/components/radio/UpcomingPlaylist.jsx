export function UpcomingPlaylist({ playlist, showOnlyOne, setShowOnlyOne, isBlurred, setIsBlurred, artPanelStyle, th, t }) {
  return (
    <div className={`${th.bgPanelCls} rounded-lg p-6 mt-6`} style={artPanelStyle}>
      <div className="flex justify-between items-center mb-4">
        <h2 className={`text-2xl font-semibold ${th.textPanel}`}>{t('upcomingSongs')}</h2>
            ...
            <div className={`flex items-center gap-3 transition-all duration-500 ${isBlurred ? 'blur-md select-none opacity-50' : 'blur-0'}`}>
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  <span className={`font-semibold block leading-tight ${th.textSecondary}`}>{track.title || track.filename}</span>
                  {track.orderType && (
                    <span className={`px-3 py-1 rounded text-[10px] font-black ${track.orderType === 'donated' ? 'bg-green-500 text-white' : 'bg-violet-500 text-white'}`}>
                      {track.orderType === 'donated'
                        ? (track.tier ? t('donateTierStatus', { tier: track.tier }) : t('donateStatus'))
                        : t('adminStatus')}
                    </span>
                  )}
                </div>
